...
class ApiKey(Base):
    ...
    __table_args__ = (
        Index('idx_ps_api_keys_hash_active', 'key_hash', 'is_active'),
    )
...
class ParsedCV(Base):
    ...
    __table_args__ = (
        Index('idx_ps_parsed_cvs_request_id', 'request_id'),
        Index('idx_ps_parsed_cvs_created_at', 'created_at'),
        Index('idx_ps_parsed_cvs_api_key_created', 'api_key_id', 'created_at'),
    )


class ScoringResult(Base):
    """CV scoring results."""
    __tablename__ = "ps_scoring_results"
    ...
    __table_args__ = (
        Index('idx_ps_scoring_results_request_id', 'request_id'),
        Index('idx_ps_scoring_results_score_created', 'overall_score', 'created_at'),
        Index('idx_ps_scoring_results_cv_job_hash', 'parsed_cv_id', 'job_description_hash'),
    )

class JobProfile(Base):
    """Job profile model for storing job requirements."""
    __tablename__ = "ps_job_profiles"
    ...
    __table_args__ = (
        Index('idx_ps_job_profiles_api_key', 'api_key_id'),
        Index('idx_ps_job_profiles_created', 'created_at'),
    )

    def __repr__(self):
        return f"<JobProfile(id={self.id}, name={self.name})>"